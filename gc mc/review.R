getwd()
setwd("D:/Introduction to R/review")
...
#read in the epilepsy, AD GWAS and SNPs
data1<- read.csv('D:/Introduction to R/review/epilepsy_age_Sex_RIN.txt',sep ='	', header= TRUE)
data2<- read.csv('D:/Introduction to R/review/Normal_age_Sex_RIN.txt',sep ='	', header= TRUE)
datacomb<-rbind(data1,data2) 
pairs(~Phenotype+Age+Sex+RIN, data = datacomb, main = 'Scatter of epi(1) and HTC(2) patients(age, sex, RIN)')
linearReg<- lm(Phenotype~Age+Sex+RIN, data = datacomb) 
linearReg

#Coefficients:
#(Intercept)          Age         SexM          RIN  
#   3.747324     0.007784     0.238703    -0.399064  

df <- data.frame(datacomb)
df_sh<- df[sample(nrow(df)),]
Ph_pred<- predict(linearReg, df_sh)
Ph_pred[Ph_pred>= mean(Ph_pred)]<- 2
Ph_pred[Ph_pred< mean(Ph_pred)]<- 1
acc<- (sum(Ph_pred[df_sh$Phenotype== 'normal'] == 2)+sum(Ph_pred[df_sh$Phenotype== 'epilepsy'] == 1))/dim(datacomb)[1]
# 0.9318182
